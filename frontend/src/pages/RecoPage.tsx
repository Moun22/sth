  ...
  return (
    <Stack gap="6">
      <PageHeader
        ...
        techs={[{ tech: 'neo4j', suffix: 'Cypher MATCH NEAR' }]}
        description={
          <>
        ...
        <Grid gridTemplateColumns="repeat(auto-fill, minmax(220px, 1fr))" gap="4">
          {recos.map((r, i) => (
            <Card.Root
              key={r.city}
              borderTop="3px solid"
              borderColor={i === 0 ? 'blue.default' : 'blue.a6'}
              bg={i === 0 ? 'blue.a2' : 'bg.default'}
            >
              <Card.Body p="5">
                <HStack gap="2" mb="2">
                  <Network size={14} color="var(--colors-blue-text)" />
                  <styled.span fontSize="xs" color="blue.text">via NEAR · graphe Neo4j</styled.span>
                </HStack>
                <Flex justify="space-between" alignItems="baseline">
                  <styled.div fontSize="3xl" fontWeight="bold" color="blue.text">{r.city}</styled.div>
                  <Badge size="sm" variant="solid" colorPalette="blue">{r.score?.toFixed(2) ?? '—'}</Badge>
                </Flex>
